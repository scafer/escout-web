...
public class AthleteService
{
    ...
    private async Task<List<Athlete>> GetAthletes(SearchQuery query)
    {
        ...
        if (query != null) request += "?query=" + JsonConvert.SerializeObject(query);
    ...
    private async Task<List<Athlete>> GetFavoriteAthletes()
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            foreach (var f in favorites) athletes.Add(await GetAthleteById(int.Parse(f.AthleteId.ToString())));
        }
